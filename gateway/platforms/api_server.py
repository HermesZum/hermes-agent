...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # --- Model lock support for OpenAI-compatible clients ---
        # When a request body contains require_model_lock: true (Open WebUI
        # custom_params), we track the requested model/provider in runtime
        # metadata; if the agent falls back to a different model we inject a
        # visible warning into the response so the user knows their picked
        # model did not answer. Fallback is NOT disabled: the turn continues
        # instead of failing mid-task.
        runtime_request = self._session_runtime_request_from_body(body)
        lock_active = bool(runtime_request.get('require_model_lock'))
        if lock_active:
            lock_route = runtime_request.get('route')
            if lock_route and not route:
                route = lock_route
            lock_requested = runtime_request.get('requested') or {}
            lock_route_source = runtime_request.get('route_source') or 'global'
        else:
            lock_requested = {}
            lock_route_source = 'global'
        ...
        if stream:
            ...
            agent_task = asyncio.ensure_future(self._run_agent(
                ...
                gateway_session_key=gateway_session_key,
                **agent_overrides,
                requested_runtime=lock_requested,
                route_source=lock_route_source,
                route=route,
            ))
        ...
        # Model-lock warning: if the client requested a specific model via
        # require_model_lock and the agent actually answered with a different
        # one (fallback chain engaged), surface it in the message content so
        # the user knows their picked model did not run.
        # Use the RESOLVED route model/provider for expected, not raw alias from requested_meta.
        if lock_active and final_response:
            runtime_meta = result.get("runtime") or {}
            expected_model = (route or {}).get("model", "")
            expected_provider = (route or {}).get("provider", "")
            actual_model = runtime_meta.get("model", "")
            actual_provider = runtime_meta.get("provider", "")
            if expected_model and actual_model and actual_model != expected_model:
                ep = f" ({expected_provider})" if expected_provider else ""
                ap = f" ({actual_provider})" if actual_provider else ""
                final_response = (
                    f"⚠️ Model '{expected_model}'{ep}"
                    f" was unavailable — "
                    f"response generated by fallback model '{actual_model}'{ap}."
                    f"\n\n"
                    + final_response
                )
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            # Track whether the fallback warning has been emitted so it
            # is injected once, ahead of the first content delta.
            warned = [False]
            ...
            async def _emit(item):
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    # Inject fallback warning before the first content delta
                    # so the user sees it at the top of the response.
                    if lock_active and not warned[0] and agent_ref[0] is not None:
                        # Read the LIVE agent model/provider — these reflect
                        # the fallback if one occurred mid-turn, unlike
                        # _hermes_api_runtime which is a creation-time snapshot.
                        actual_model = getattr(agent_ref[0], "model", "") or ""
                        actual_provider = getattr(agent_ref[0], "provider", "") or ""
                        # Use RESOLVED route model/provider for expected, not raw alias from lock_requested
                        expected_model = (route or {}).get("model", "")
                        expected_provider = (route or {}).get("provider", "")
                        if expected_model and actual_model and actual_model != expected_model:
                            # Clean provider/model names for display
                            ep = self._clean_runtime_id(expected_provider, max_len=80) if expected_provider else ""
                            ap = self._clean_runtime_id(actual_provider, max_len=80) if actual_provider else ""
                            em = self._clean_runtime_id(expected_model) if expected_model else ""
                            am = self._clean_runtime_id(actual_model) if actual_model else ""
                            warn_text = (
                                f"⚠️ Model '{em}'"
                                f"{f' ({ep})' if ep else ''}"
                                f" was unavailable — "
                                f"response generated by fallback model '{am}'"
                                f"{f' ({ap})' if ap else ''}."
                                f"\n\n"
                            )
                            warn_chunk = {
                                ...
                                "choices": [{
                                    "index": 0,
                                    "delta": {"content": warn_text},
                                    "finish_reason": None
                                }],
                            }
                            await response.write(_sse_frame(warn_chunk))
                        warned[0] = True
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
